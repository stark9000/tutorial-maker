...
public class ArrowShape extends AnnotationShape {
    public float fx1,fy1,fx2,fy2;
    public ArrowShape(float x1,float y1,float x2,float y2,Color c,float t){super(c,t);fx1=x1;fy1=y1;fx2=x2;fy2=y2;}
    @Override public void draw(Graphics2D g2,int w,int h){
        int x1=(int)(fx1*w),y1=(int)(fy1*h),x2=(int)(fx2*w),y2=(int)(fy2*h);
        g2.setColor(color);g2.setStroke(new BasicStroke(thickness,BasicStroke.CAP_ROUND,BasicStroke.JOIN_ROUND));
        g2.drawLine(x1,y1,x2,y2);
        double a=Math.atan2(y2-y1,x2-x1);int sz=(int)(thickness*4+8);
        g2.drawLine(x2,y2,(int)(x2-sz*Math.cos(a-Math.PI/6)),(int)(y2-sz*Math.sin(a-Math.PI/6)));
        g2.drawLine(x2,y2,(int)(x2-sz*Math.cos(a+Math.PI/6)),(int)(y2-sz*Math.sin(a+Math.PI/6)));
